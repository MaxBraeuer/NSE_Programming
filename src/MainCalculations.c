...
static void calculate_residuals(SimulationGrid **cell, int i_max, int j_max, double delta_x, double delta_y) {
  for (int i = 1; i <= i_max; i++) {
    for (int j = 1; j <= j_max; j++) {
      cell[i][j].residual = ((cell[i+1][j].pressure - 2.0 * cell[i][j].pressure + cell[i-1][j].pressure) / (delta_x * delta_x) +
          (cell[i][j+1].pressure - 2.0 * cell[i][j].pressure + cell[i][j-1].pressure) / (delta_y * delta_y) - cell[i][j].poisson_rhs);
    }
  }
}
